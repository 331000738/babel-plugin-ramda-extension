...
const _ramdaPath = path.dirname(Module._resolveFilename('ramda-extension', merge(new Module, {
  'paths': Module._nodeModulePaths(process.cwd())
})));

// ramda folder will be /nodemodules/ramda/dist. We want to remove the dist
const ramdaPath = _ramdaPath.slice(0, _ramdaPath.lastIndexOf('ramda-extension') + 15);
...
export default function resolveModule(useES, name) {
  ...
  for (var category in methods) {
    if (contains(name, methods)) {
      return `ramda-extension/lib/${name}`;
    }
  }
  throw new Error(`Ramda method ${name} was not a known function
    Please file a bug if it's my fault https://github.com/331000738/babel-plugin-babel-plugin-ramda-extension/issues
  `);
};
