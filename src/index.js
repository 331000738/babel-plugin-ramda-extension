...
export default function({ types: t }) {
  ...
  // Import a ramda-extension method and return the computed import identifier
  ...
  return {
    visitor: {
      Program: {
        enter() {
          // Track the variables used to import ramda-extension
      ...
      ImportDeclaration(path) {
        let { node } = path;
        if (node.source.value === 'ramda-extension') {
      ...
      ExportNamedDeclaration(path, state) {
        ...
        if (node.source && node.source.value === 'ramda-extension') {
      ...
      ExportAllDeclaration(path) {
        let { node } = path;
        if (node.source && node.source.value === 'ramda-extension') {
          throw new Error('`export * from "ramda-extension"` defeats the purpose of ramda-extension');
        }
      },
      ...
      Identifier(path, state) {
        ...
        if (matchesRamdaMethod(path, name) && !isSpecialTypes(t, parent)) {
          ...
          // #19, nullify direct references to the ramda-extension import (for apply/spread/etc)
